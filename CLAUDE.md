...
- **SEO:** `src/content/seo.json` (siteUrl, default + per-route `{es,en}`
  ...
  **Since routes are language-prefixed (FCR-106), `resolveSeo`'s canonical is now
  the language-prefixed `/<lang>/<slug>` URL** (via `localizedPath`), matching the
  real `/:lang` route + the Layer-1 prerender; hreflang alternates were already
  prefixed. `scripts/prerender.mjs` runs after `vite build` (chained in the
  `build` script) → per-language×route static HTML under `dist/<lang>/<slug>/` +
  `sitemap.xml` + a noindex `404.html`; it ALSO **overwrites the root
  `dist/index.html` with a tiny meta-refresh redirect shell to `/es`** (so the
  bare `/` lands on the default language — the SPA still hydrates client routes
  via the `404.html` fallback). **When you add/rename a public route:** add it to
  `seo.json.pages` AND the `ROUTES` list in `prerender.mjs`. The deploy workflow
  must NOT `cp index.html→404.html` (it would clobber the noindex 404 the
  prerender emits AND the root redirect shell).
